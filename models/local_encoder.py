...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # self.attn_drop_edge = AttnDropEdge(torch.pi/2)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        if self.parallel:
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                head_dir = data['rotate_angles'][edge_index[0]] - data['rotate_angles'][edge_index[1]]
                out[t] = self.aa_encoder(x=data.x[:, t], t=t, edge_index=edge_index, edge_attr=edge_attr,head_dir=head_dir,
...
class AAEncoder(MessagePassing):
    ...
    def __init__(self,
        ...
        self.nbr_embed = MultipleInputEmbedding(in_channels=[node_dim, edge_dim, 1], out_channel=embed_dim)
    ...
    def forward(self,
        ...
        center_embed = center_embed + self._mha_block(self.norm1(center_embed), x, edge_index, edge_attr, head_dir, rotate_mat,
                                                      size)
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr,head_dir=head_dir, rotate_mat=rotate_mat, size=size))
